...
export async function GET(request: NextRequest, { params }: { params: { clientName: string } }) {
  ...
  try {
    ...
    const checkinMap = new Map<number, CMCheckinWithGoals>()
    ...
    console.error("Failed to fetch CM check-ins by client:", error)
    return NextResponse.json(
      {
        success: false,
        error: {
          code: "DATABASE_ERROR",
          message: "Failed to fetch CM check-ins",
