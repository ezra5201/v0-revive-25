...
export async function GET(request: NextRequest, { params }: { params: { clientName: string } }) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            details: { field: "clientName", value: params.clientName },
          },
        },
        { status: 400 },
      )
    ...
    const result = await sql`
      ...
      FROM ot_checkins c
      LEFT JOIN ot_goals g ON c.id = g.checkin_id
    ...
    const checkinMap = new Map<number, OTCheckinWithGoals>()
    ...
    console.error("Failed to fetch OT check-ins by client:", error)
    return NextResponse.json(
      {
        success: false,
        error: {
          code: "DATABASE_ERROR",
          message: "Failed to fetch OT check-ins",
